 ...
 *         Time: 10:38
 */
public class AppointmentDbToUiConverter implements Converter<AppointmentDb, AppointmentUi> {

    @Autowired
    private ConversionServiceFactoryBean conversionServiceFactoryBean;

    @Override
    public AppointmentUi convert(AppointmentDb src) {
        ConversionService conversionService = conversionServiceFactoryBean.getObject();
        AppointmentUi dest = new AppointmentUi();

        dest.setId(src.getId());
        dest.setCreated(src.getCreated());
        dest.setPlanned(src.getPlanned());
        dest.setPatient(conversionService.convert(src.getPatient(), UserUi.class));
        dest.setDoctor(conversionService.convert(src.getDoctor(), UserUi.class));
